...
def init_params(name, in_channels, kernel_size): # only for 1D conv
...
class BMN(paddle.nn.Layer):
    ...
    def __init__(
        ...
        #===========xzq added==============
        self.p_conv = paddle.nn.Sequential(
            paddle.nn.Conv1D(
                in_channels=self.feat_dim,
                out_channels=self.d_model,
                kernel_size=1,
                padding=0,
                weight_attr=ParamAttr(name="ADD_1d_w"),
                bias_attr=ParamAttr(name="PEM_1d_b1")
                        ),
            paddle.nn.ReLU()
        )
        ...
        #Proposal Evaluation Module  refine
        ...
        self.p_conv3d1 = paddle.nn.Conv3D(
            ...
            padding=0,
            weight_attr=ParamAttr(name="PEM_3d1_w"), # weight initialized by kaiming_normal default
            bias_attr=ParamAttr(name="PEM_3d1_b")) # bias should be initialized by 1/math.sqrt(fan)
        self.p_conv3d1_act = paddle.nn.ReLU()

        self.p_conv2d1 = paddle.nn.Conv2D(
            in_channels=self.hidden_dim_3d,
...
if __name__ == '__main__':
    ...
    bmn = BMN(100,100,0.5,32,3,400)
    x = paddle.randn([16,400,100], dtype='float32')
    output = bmn(x)
    print(len(output))
